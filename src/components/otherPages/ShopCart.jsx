            ...
            <div className="col-xl-8">
              ...
              {cartProducts.length ? (
                        ...
                        <tr key={i} className="tf-cart-item file-delete">
                          ...
                          >
                            <div className="cart-price text-button price-on-sale">
                              <span className="uae-icon" title="aed" /> {" "} {elm.price.toFixed(2)}
                            </div>
                          </td>
                          <td
                          ...
                          >
                            <div className="cart-total text-button total-price">
                              <span className="uae-icon" title="aed" /> {" "} {(elm.price * elm.quantity).toFixed(2)}
                            </div>
                          </td>
                          <td
                ...
                <div>
                  ...
                  save them for later!{" "}
                  <Link className="btn-line" to={`/all-collections`}>
                    Explore Products
                  </Link>
                </div>
              )}
              ...
              <div className="fl-sidebar-cart">
                <div className="box-order bg-surface">
                  ...
                  <div className="subtotal text-button d-flex justify-content-between align-items-center">
                    <span>Subtotal</span>
                    <span className="total"><span className="uae-icon" title="aed" /> {" "} {(totalPrice + discount).toFixed(2)}</span>
                  </div>
                  <div className="discount text-button d-flex justify-content-between align-items-center">
                    <span>Shipping</span>
                    <span className="total"><span className="uae-icon" title="aed" /> {" "}  0</span>
                  </div>
                  <div className="discount text-button d-flex justify-content-between align-items-center">
                    <span>Discounts</span>
                    <span className="total">-<span className="uae-icon" title="aed" /> {" "} {discount}</span>
                  </div>
                  {/* <div className="ship">
                  ...
                  <div className="box-progress-checkout">
                    ...
                    <Link
                      to={`/checkout`}
                      className={`tf-btn btn-reset ${!isChecked || totalPrice <= 0 ? "disabled" : ""}`}
                      onClick={(e) => {
                        if (!isChecked || totalPrice <= 0) e.preventDefault();
                      }}
                    >
                      Process To Checkout
                    </Link>
                    <p className="text-button text-center">
                      <Link to={`/all-collections`}> Or continue shopping
                      </Link>
                    </p>
                  </div>
                </div>
